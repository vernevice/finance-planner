...
1. **`policy.yaml` has no values.** Every household decision is set to
   `REQUIRED`. The two that matter most are `buffer.months` and
   `investment.risk_premium_bps` — see
   [0001](docs/decisions/0001-policy-values-pending.md).
2. **Every value in `tax/FY2027.yaml` is `UNVERIFIED`.** `ato.gov.au` was
   unreachable from the environment where this was scaffolded, so nothing
   could be checked against source — see
   [0002](docs/decisions/0002-tax-values-unverified.md).
3. **Raw data is gitignored by default**, which leaves §2.3 unenforced —
   see [0004](docs/decisions/0004-raw-data-not-committed.md).
